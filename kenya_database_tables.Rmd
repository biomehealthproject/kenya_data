...
all_exif$filepath<-gsub("\\\\","/",all_exif$filepath)
...
all_exif_fp<-merge(fp, all_exif, by = "filepath")

sel_exif<-all_exif_fp%>%
  select(Site.ID,Site.x, Location.ID,Camera.ID,Season.x ,date,datetime,image_no.x,ab,CommonName,Confidence, new_file_structure)

colnames(sel_exif)<-c("SiteID", "Site", "LocationID", "CameraID", "Season", "Date", "DateTime","ImageNo","AB",  "CommonName","Confidence", "FileLocation")


#write.csv(sel_exif, here::here("database/kenya_data/ImageTbl.csv"), row.names = FALSE)
